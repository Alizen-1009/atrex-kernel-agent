...
```bash
python tools/iteration_trace.py phase-start <profile|research|planning|implementation|correctness|benchmark>
python tools/iteration_trace.py phase-end <profile|research|planning|implementation|correctness|benchmark>
```

Map the work as follows: Stage 1 is `profile`; source investigation in Step A or Stage 2 is `research`; selecting and documenting the one lever in Stage 2 is `planning`; Stage 3 is `implementation`; correctness checks in Stage 4 are `correctness`; and final performance measurement in Stage 4 is `benchmark`.

Use complete marker pairs around the corresponding work below. Execute each `phase-start` marker as a standalone tool call and wait for its receipt before issuing that phase's work; execute `phase-end` only after the work completes. At most one phase may be active at a time: end the current phase before starting another. A phase may be entered again later with a new complete pair. Missing, unclosed, nested, or overlapping markers are left unattributed rather than guessed. Before or immediately after reading a relevant source, record only its metadata (never its contents):
...
Do not record credentials, private URL parameters, absolute user paths, raw tool output, or transcript text. If the helper is unavailable or returns an error, continue the optimization normally and leave telemetry coverage partial.
