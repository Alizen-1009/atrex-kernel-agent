...
This route runs the optimization loop from the source repo without installing anything into your coding runtime. `orchestrator/optimize.py` owns the **outer loop** and spawns a fresh, clean Claude, Qoder, Codex, or Pi CLI session for each iteration. Select the backend with `--agent-cli claude|qodercli|codex|pi` (default: `claude`). State crosses the session boundary only through disk (`memory/v<N>.json`, `plans/`, `profiles/`, and git), and HEAD is always the best kernel. Codex runs use `codex exec --json` with a fresh thread. Ordinary iterations run inside an isolated temporary `CODEX_HOME` that links the existing auth/config/skills but owns all newly written rollout and state files; the entire temporary home is removed after either successful normalization or terminal-only fallback. The orchestrator reads rollout `session_meta` only to recover the exact workspace/thread when stdout omits it, and accounting reads only structured token-count and explicit marker records. Cleanup failures are observation reason codes and never change the Agent result. Repository-scoped skills remain under each campaign's `.agents/skills/` without modifying the user's global Codex skills.
...
Codex JSONL `turn.completed.usage` remains the cumulative session total, while per-turn deltas come from the matching native rollout. Every available input/output/cache/total component must reconcile before phase attribution. Ordinary Codex rollouts are deleted after each observation attempt; long-horizon rollouts remain available for bounded same-thread resume and are read incrementally. A resume-time ledger failure derives the invocation budget delta from consecutive cumulative stdout totals instead of double-counting the session. Cache and reasoning sub-counters are not double-counted. Pi finalized message usage, including cache read/write counters,
